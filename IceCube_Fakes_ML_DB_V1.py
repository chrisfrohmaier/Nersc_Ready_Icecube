...
def file_structure(): #This definition creates the file structure for results to be added into
	...
	if not os.path.exists('Results_V1'):
		os.makedirs('Results_V1')
	if not os.path.exists('Results_V1/Catalog'):
		os.makedirs('Results_V1/Catalog')
	if not os.path.exists('Results_V1/Fake_Star_Catalog'):
		os.makedirs('Results_V1/Fake_Star_Catalog')
	if not os.path.exists('Results_V1/Fakes_added'):
		os.makedirs('Results_V1/Fakes_added')	
	if not os.path.exists('Results_V1/Galaxies'):
		os.makedirs('Results_V1/Galaxies')

def Sextract(science_image,zeropoint,seeing,saturation,gain): #Runs Sextractor and creates a catalog of all the stars
	
	subprocess.call('sex -c Pipe_sexfile_CMD.sex '+science_image[0]+science_image[1]+'.fits -PARAMETERS_NAME PTF_Transform_Param.param -FILTER_NAME default.conv -CATALOG_NAME Results_V1/Catalog/'+science_image[1]+'_Catalog_V1.cat -WEIGHT_IMAGE '+science_image[0]+science_image[1]+'.weight.fits -MAG_ZEROPOINT'+'	'+str(zeropoint)+' -SEEING_FWHM '+str(seeing)+' -SATUR_LEVEL '+str(saturation)+' -GAIN '+str(gain)+' -VERBOSE_TYPE QUIET',shell=True)

def Enough_Objects(science_image): #Checks that sextractor has found at least 200 objects
	enough=True
	test=os.popen('wc -l Results_V1/Catalog/'+science_image[1]+'_Catalog_V1.cat').read()
	rows=test.split()
...
def Selecting_Bright(science_image): #Selected the top 20 brightest stars in the catalog
	f=open('Results_V1/Catalog/'+science_image[1]+'_Catalog_V1.cat') #CHANGE THIS SO THAT IT CAN TAKE ANY CATALOG INPUT
	fin=f.readline()
...
def selecting_galaxies(science_image,): #Finds and creates a catalog of Galxies
	...
	f=open('Results_V1/Catalog/'+science_image[1]+'_Catalog_V1.cat')
	g=open('Results_V1/Galaxies/'+science_image[1]+'_Galaxy_Catalog_V1.cat','w')
	l=open('Results_V1/Galaxies/'+science_image[1]+'_Galaxy_regions_V1.reg','w')
	m=open('Results_V1/Galaxies/'+science_image[1]+'_Star_regions_V1.reg','w')
	fin=f.readline()
	X2_array=[]
	Y2_array=[]
	FWHM_array=[]
...
def add_fakes_2galaxy(science_image,boxsize, xcord_star, ycord_star, newx_star, newy_star, mag_array_star, flux_array_star, ran_mag_star, ran_flux_star, background_array_star, scaling_factor_star, CCD_Num_star,  mag_best_star, alpha_array, delta_array, zeropoint, seeing, saturation, gain, readnoise, MOONILLF, AIRMASS, ELLIP, MEDSKY, SKYSIG,LMT_MG, MJD,  MoonRA, MoonDec, PTFFIELD):
	...
	h=open('Results_V1/Galaxies/'+science_image[1]+'_Galaxy_Catalog_V1.cat') #Opens the Galaxy catalog
	f=open('Results_V1/Fake_Star_Catalog/'+science_image[1]+'_Fake_Star_Catalog_V1.dat','w') #Opens the fake star catalog
	reg=open('Results_V1/Fake_Star_Catalog/'+science_image[1]+'_Fakes_Star_Regions_V1.reg','w') #creates region file
	regworld=open('Results_V1/Fake_Star_Catalog/'+science_image[1]+'_Fakes_Star_Regions_V1_World.reg','w')
	Galworld=open('Results_V1/Fake_Star_Catalog/'+science_image[1]+'_Galaxy_Regions_V1_Sex.reg','w')
	...
	num_of_gal_fakes=0
	j=open('Results_V1/Fakes_added/'+science_image[1]+'_Flux_Boxes_V1.dat','w')
	...
	hdulist_sci.writeto(science_image[0]+science_image[1]+'_fakesV1.fits', output_verify='ignore', clobber=True) #Saving image after loop of 200 Stars is complete
	j.close()
	reg.close()	
	f.close()
...
def Execute(run):
	...
	try:
		...
		bad_images=open('Results_V1/Bad_Images_V1.dat','a')
	...
	fake_stars= 0 #number of fake stars per image (integer please!)
	...
	if catsize==False:
			print science_image, 'didn\'t have enough objects detected so it was moved to Results_V1/Bad_Images/ and the newly created weight map, sex file and catalog have been deleted'
			bad_images=open('Results_V1/Bad_Images_V1.dat','a')
			...
			os.remove('Results_V1/Catalog/'+science_image[1]+'_Catalog_V1.cat')
			return
	...
	#print 'Scaling Done'
	mag_log=open('Results_V1/Magnitude_Log_File.dat','a')
	...
	good_images=open('Results_V1/Good_Images_V1.dat','a')
...
def Run_All_V1():
	...
	#print len(science_fits)

	bad_images=open('Results_V1/Bad_Images_V1.dat','w')
	bad_images.close()
	good_images=open('Results_V1/Good_Images_V1.dat','w')
	good_images.close()
	mag_log=open('Results_V1/Magnitude_Log_File.dat','w')
	mag_log.close()
	mag_log_col=open('Results_V1/Magnitude_Log_File.columns','w')
	...
	print 'V1 took: ', time.time()-t0, 'seconds'
